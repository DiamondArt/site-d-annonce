...
$titre=$message=$fichier=$fichierError =$id_destinataire=$messageErreur=$erreur="";
 ...
 if(isset($_SESSION['id']) AND !empty($_SESSION['id']))// la session en cours
 {
	 ...
	 $select = $db->prepare("SELECT reponse.id,reponse.message,reponse.destinataire,reponse.id_expediteur,reponse.id_destinataire,
	 reponse.sujet,reponse.expediteur FROM reponse WHERE reponse.id = ? ");
