...
$titre=$message=$fichier=$fichierError =$id_destinataire=$messageErreur=$erreur="";
 ...
 {
	 $db = Database::connect();
	 $select = $db->query("SELECT message.id,message.destinataire,message.expediteur_id, message.sujet,message.expediteur_firstname FROM message WHERE message.id ='{$_GET['id']}' ");
	 $info= $select->fetch();
	 $id_msg =$info['id'];
	 $id_destinataire=$info['expediteur_id'];
	 $destinataire= $info['expediteur_firstname'];
	 Database::disconnect();
...
<section class="container " >
 <center class="row">
  <div class="container-register ">
    <form class="col-md-8 form-group " action="" method="POST" enctype="multipart/form-data">
	 <p class="info mb-2">
	...
	<i class="help-inline text-info"> <b class="text-danger">info : </b>en cas d'envoie de fichier,veuillez mettre tous vos fichiers (images, pdf...) dans un fichier zip!</i>
				...
				<div><p> Répondre à Mr (Mme): <b><?php echo strtoupper( $destinataire);?></b></p> </div>
						...
						<span class="input-group-text">SUJET</span>						     <input disabled class="form-control" type="text" name="titre" id="titre" value="<?php echo $info['sujet'];?>" >
					         </div>
							  <div class="wrap-input100 input-group" >
							    ...
							    <div class="wrap-input100">
								  ...
								  <a href="../dashboard/table.php" class="btn-lg btn-block btn-primary form-control">Retour</a>
					   </div>
				   </form>
		  		</div>
			</center>
